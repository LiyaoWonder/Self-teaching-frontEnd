...
var Student = function(name, height) {
    this.name = name
    // 用 this.变量名 来创建一个只有类的实例才能访问的变量
    this.height = height
}

// 对象需要 new
var ob1 = new Student('guaI', 179)
var ob2 = new Student('guaII', 179)
var ob3 = new Student('guaIII', 179)

log('class ob1', ob1.name, ob1.height)

// obj vs ob1 类中可以绑定函数 obj这种直接创建对象是做不到的

// 可以给类增加一些方法（函数）
// 给类定义方法（想想 标准库中 string 类的length）
// string 就是类的属性
...
ob1.update('Wonder', '18岁')

// prototype是加在类上的 所有的实例都共享这个函数
// 以下就只有 ob1 有
ob1.hello = function() {
    log('Hello')
}
...
/*
类就是很自然而然的封装
意思就是把一些操作做好， 对外部来说只是简单调用即可

类的最大优势是可以批量制造 object 和 可封装方法

类，一群有着相同属性和相同功能（函数）的对象的集合
*/